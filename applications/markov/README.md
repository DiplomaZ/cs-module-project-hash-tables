...
#word in Words
#attempt to add but first compare word sequence (store words in arr?) with current sequence. if word len is less than existing, do nothing. if it is the same, do nothing. if the dynamically grown array is the same as checked against, paste in master
